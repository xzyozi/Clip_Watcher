...
# 修正方針 (Correction Policy)

## 1. 履歴の保存 (History Persistence)

### 採用方針: デバウンス付き自動保存 (Debounced Auto-Save)
パフォーマンスとデータ保全のバランスを最適化するため、**デバウンス（Debounce）方式**を採用します。

- **設計責務**: `ClipboardMonitor` が自身のデータの永続化を管理します。
- **実装内容**:
    - 履歴に変更があった場合（追加・削除・編集・ピン留め）、即時保存するのではなく `schedule_save()` を呼び出します。
    - `schedule_save()` は既存のタイマーがあればキャンセルし、指定時間（例: 3秒後）に `save_history_to_file()` を実行するようスケジュールします。
    - これにより、連続したコピー操作時などの頻繁なディスク書き込みを防ぎつつ、操作終了後すぐに自動保存される状態を作ります。
    - アプリ終了時(`shutdown`)には、待機中の保存処理をキャンセルして即時保存を行います。

## 2. GUI初期表示 (Initial GUI Loading)

### 採用方針: ライフサイクルによる明示的初期化
アプリケーション起動完了時に、Controller層である `MainApplication` が明示的に View を更新します。

- **設計責務**: `MainApplication` が `on_ready` イベントにおいて Model と View の同期を行います。
- **実装内容**:
    - `src/core/app_main.py` の `on_ready` メソッド内で、監視スレッド開始前に `self.update_gui(...)` を呼び出します。
    - これにより、`monitor` がメモリ上に持っている読み込み済み履歴データが即座に GUI に反映されます。
