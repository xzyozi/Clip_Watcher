    ...
    def _add_menu_items(self, state: MenuState):
        ...
        self.menu.add_command(
            ...
            command=lambda: self.dispatcher.dispatch(
                "HISTORY_COPY_SELECTED", state.selection
            ),
        )
        # ... その他の項目
...
class ComponentValidator:
    ...
    def validate(builder: "ApplicationBuilder") -> None:
        ...
        missing = [
            comp
            for comp in ComponentValidator.REQUIRED_COMPONENTS
            if getattr(builder, comp) is None
        ]
        if missing:
            raise ConfigError(f"必須コンポーネントが未初期化: {', '.join(missing)}")
