...
def create_menu_bar(master: tk.Tk, app_instance: BaseApplication) -> tk.Menu:
    ...
    file_menu.add_command(
        label=translator("settings_menu_item"),
        command=app_instance.open_settings_window,
    )  # type: ignore
    file_menu.add_command(
        label=translator("export_history_menu_item"),
        command=app_instance.file_handlers.handle_export_history,
    )  # type: ignore
    file_menu.add_command(
        label=translator("import_history_menu_item"),
        command=app_instance.file_handlers.handle_import_history,
    )  # type: ignore
    file_menu.add_separator()
    file_menu.add_command(
        label=translator("exit_menu_item"),
        command=app_instance.file_handlers.handle_quit,
    )  # type: ignore
    ...
    edit_menu.add_command(
        label=translator("find_menu_item"), command=lambda: logger.info("Find clicked")
    )
    edit_menu.add_command(
        label=translator("copy_merged_menu_item"),
        command=lambda: app_instance.event_dispatcher.dispatch(
            "HISTORY_COPY_MERGED",
    ...
    edit_menu.add_command(
        label=translator("clear_all_history_menu_item"),
        command=app_instance.history_handlers.handle_clear_all_history,
    )  # type: ignore
    ...
    app_instance.always_on_top_var = tk.BooleanVar(
        value=app_instance.settings_manager.get_setting("always_on_top")
    )  # type: ignore
    view_menu.add_checkbutton(
        label=translator("always_on_top_menu_item"),
        command=lambda: app_instance.event_dispatcher.dispatch(
            "SETTINGS_ALWAYS_ON_TOP", app_instance.always_on_top_var.get()
        ),  # type: ignore
        variable=app_instance.always_on_top_var,
    )  # type: ignore

    app_instance.theme_var = tk.StringVar(
        value=app_instance.settings_manager.get_setting("theme")
    )  # type: ignore
    theme_menu = tk.Menu(view_menu, tearoff=0)
    theme_menu.add_radiobutton(
        label=translator("light_theme_menu_item"),
        variable=app_instance.theme_var,
        value="light",  # type: ignore
        command=lambda: app_instance.settings_handlers.handle_set_theme(
            "light", save=False
        ),
    )  # type: ignore
    theme_menu.add_radiobutton(
        label=translator("dark_theme_menu_item"),
        variable=app_instance.theme_var,
        value="dark",  # type: ignore
        command=lambda: app_instance.settings_handlers.handle_set_theme(
            "dark", save=False
        ),
    )  # type: ignore
    theme_menu.add_radiobutton(
        label=translator("system_theme_menu_item"),
        variable=app_instance.theme_var,
        value="system",  # type: ignore
        command=lambda: logger.info(
            "Follow System theme clicked (not yet implemented)"
        ),
    )
    ...
    filter_menu.add_command(
        label=translator("show_all_menu_item"),
        command=lambda: logger.info("Show All clicked"),
    )
    filter_menu.add_command(
        label=translator("show_text_only_menu_item"),
        command=lambda: logger.info("Show Text Only clicked"),
    )
    filter_menu.add_command(
        label=translator("show_images_only_menu_item"),
        command=lambda: logger.info("Show Images Only clicked"),
    )
    ...
    for plugin in gui_plugins:
        ...
        tools_menu.add_command(
            label=translator(plugin.name), command=select_plugin_tab_command
        )
    ...
    help_menu.add_command(
        label=translator("how_to_use_menu_item"),
        command=event_handlers.handle_how_to_use,
    )  # type: ignore
    help_menu.add_command(
        label=translator("about_menu_item"), command=event_handlers.handle_about
    )  # type: ignore
