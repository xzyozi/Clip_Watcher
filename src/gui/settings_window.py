...
class SettingsWindow(tk.Toplevel):
    def __init__(self, master, settings_manager, plugin_manager, app_instance):
        ...
        for plugin in self.plugin_manager.get_available_plugins():
            ...
            self.plugin_vars[plugin_key] = tk.BooleanVar(value=plugin_settings.get(plugin_key, False))
    ...
    def _create_widgets(self):
        ...
        plugins_frame = ttk.Frame(notebook, padding=config.FRAME_PADDING)
        ...
        plugins_list_frame = ttk.LabelFrame(plugins_frame, text="Available Plugins", padding=config.FRAME_PADDING)
        plugins_list_frame.pack(fill=tk.X, pady=config.BUTTON_PADDING_Y, padx=config.BUTTON_PADDING_X)
        for i, plugin in enumerate(self.plugin_manager.get_available_plugins()):
            plugin_key = plugin.__class__.__name__
            checkbutton = ttk.Checkbutton(plugins_list_frame, text=plugin.name, variable=self.plugin_vars[plugin_key])
            checkbutton.grid(row=i, column=0, sticky=tk.W, padx=config.BUTTON_PADDING_X)
            description_label = ttk.Label(plugins_list_frame, text=f"- {plugin.description}")
            description_label.grid(row=i, column=1, sticky=tk.W, padx=config.BUTTON_PADDING_X)

        # IO, Save, Cancel, Apply Buttons
    ...
    def _save_settings_logic(self):
        ...
        plugin_settings = {key: var.get() for key, var in self.plugin_vars.items()}
        self.settings_manager.set_setting("plugins", plugin_settings)
    ...
    def _update_ui_from_settings(self):
        ...
        plugin_settings = self.settings_manager.get_setting("plugins", {})
        for plugin_key, var in self.plugin_vars.items():
            var.set(plugin_settings.get(plugin_key, False))
