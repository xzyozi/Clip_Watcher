...
class HistoryEventHandlers(BaseEventHandler):
    ...
    def _register_handlers(self) -> None:
        ...
        self.subscribe(
            "HISTORY_DELETE_ALL_UNPINNED", self.handle_delete_all_unpinned_history
        )
    ...
    def handle_history_item_edited(self, data: dict[str, Any]) -> None:
        try:
            ...
            logger.warning(
                "Could not update history item: No item selected or ID not found."
            )
        except Exception as e:
            log_and_show_error(
                "Error", f"Failed to update history item: {e}", exc_info=True
            )
    ...
    def handle_create_quick_task(self, item_ids: list[float]) -> None:
        ...
        tasks: list[str] = [
            id_to_content[item_id] for item_id in item_ids if item_id in id_to_content
        ]
    ...
    def handle_delete_all_unpinned_history(self) -> None:
        if messagebox.askyesno(
            ...
            messagebox.showinfo(
                "完了",
                "ピン留めされていない履歴をすべて削除しました。",
                parent=self.app.master,
            )  # type: ignore
        else:
            messagebox.showinfo(
                "キャンセル", "操作をキャンセルしました。", parent=self.app.master
            )  # type: ignore
    ...
    def handle_pin_unpin_history(self, item_id: float | None) -> None:
        ...
        try:
            ...
            if item_tuple is None:
                logger.error(
                    f"Could not find history item with ID {item_id} for pin/unpin."
                )
                return
    ...
    def handle_copy_selected_as_merged(self, item_ids: list[float]) -> None:
        ...
        try:
            history_data: list[tuple[str, bool, float]] = self.app.monitor.get_history()  # type: ignore
            id_to_content: dict[float, str] = {
                item[2]: item[0] for item in history_data
            }
            ...
            logger.error(
                f"Error merging and copying selected history: {e}", exc_info=True
            )

    def format_selected_item(self) -> None:
        try:
            selected_indices: tuple[int, ...] = (
                self.app.gui.history_component.get_selected_indices()
            )  # type: ignore
            if not selected_indices:
                return
            ...
            dialog: FormatDialog = self.app.create_toplevel(
                FormatDialog, self.app.settings_manager
            )  # type: ignore
            ...
            log_and_show_error(
                "エラー", "フォーマット対象の項目が選択されていません。", exc_info=True
            )
        except Exception as e:
            log_and_show_error(
                "エラー",
                f"フォーマット中に予期せぬエラーが発生しました。\n\n{e}",
                exc_info=True,
            )
        ...
        try:
            ...
            if 0 <= selected_index < len(history_data):
                ...
                if processed_text != original_text:
                    ...
                    self.undo_manager.execute_command(command)
                    logger.info(
                        f"Executed format command for item with ID {item_id} with {plugin_instance.name}"
                    )
                else:
    ...
    def handle_search_history(self, search_query: str) -> None:
        if search_query:
            ...
            self.app.gui.update_clipboard_display(
                self.app.monitor.last_clipboard_data, filtered_history
            )  # type: ignore
        else:
            self.app.gui.update_clipboard_display(
                self.app.monitor.last_clipboard_data, self.app.monitor.get_history()
            )  # type: ignore
