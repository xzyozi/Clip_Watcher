...
class FileEventHandlers(BaseEventHandler):
    def __init__(
        self, app_instance: BaseApplication, event_dispatcher: EventDispatcher
    ) -> None:
    ...
    def handle_export_history(self) -> None:
        ...
        if file_path:
            try:
                history_content: list[tuple[str, bool, float]] = (
                    self.app.monitor.get_history()
                )  # type: ignore
                ...
                messagebox.showinfo(
                    "エクスポート完了",
                    f"履歴を以下のファイルにエクスポートしました:\n{file_path}",
                )
            except Exception as e:
                messagebox.showerror(
                    "エクスポートエラー",
                    f"履歴のエクスポート中にエラーが発生しました:\n{e}",
                )
        ...
        if file_path:
            try:
                ...
                with open(file_path, encoding="utf-8") as f:
                    ...
                    imported_history = [
                        item.strip() for item in raw_items if item.strip()
                    ]

                self.app.monitor.import_history(imported_history)  # type: ignore
                messagebox.showinfo(
                    "インポート完了",
                    f"履歴を以下のファイルからインポートしました:\n{file_path}",
                )
            except Exception as e:
                messagebox.showerror(
                    "インポートエラー",
                    f"履歴のインポート中にエラーが発生しました:\n{e}",
                )
