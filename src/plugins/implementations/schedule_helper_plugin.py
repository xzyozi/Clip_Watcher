...
class ScheduleHelperComponent(BaseFrameGUI):
    ...
    def __init__(self, master: tk.Misc, app_instance: BaseApplication) -> None:
        ...
        self.app.event_dispatcher.subscribe(
            "LANGUAGE_CHANGED", self._apply_locale_settings
        )  # type: ignore
    ...
    def _create_widgets(self) -> None:
        ...
        ttk.Button(nav_frame, text="<", command=self._prev_month, width=3).pack(
            side=tk.LEFT
        )
        self.month_year_label = ttk.Label(nav_frame, text="", width=18, anchor="center")
        self.month_year_label.pack(side=tk.LEFT, padx=5)
        ttk.Button(nav_frame, text=">", command=self._next_month, width=3).pack(
            side=tk.LEFT
        )
        ttk.Button(nav_frame, text="Today", command=self._go_to_today).pack(
        ...
        ttk.Button(
            button_bar_frame, text="Copy to Clipboard", command=self._copy_to_clipboard
        ).pack(side=tk.RIGHT)
        ttk.Button(button_bar_frame, text="Clear", command=self._clear_text).pack(
    ...
    def _update_calendar(self) -> None:
        ...
        for i, day in enumerate(days):
            ttk.Label(self.calendar_frame, text=day, width=5, anchor="center").grid(
    ...
    def _select_date(self, day: int) -> None:
        ...
        for d in self.selected_dates:
            if (
                d.year == new_date_for_check.year
                and d.month == new_date_for_check.month
                and d.day == new_date_for_check.day
            ):
                found_date = d
                break
    ...
    def _copy_to_clipboard(self) -> None:
        ...
        try:
            ...
            messagebox.showinfo(
                "Copied", "テキストをクリップボードにコピーしました。", parent=self
            )
        except tk.TclError:
            self.logger.error("Failed to copy text to clipboard.")
            messagebox.showerror(
                "Error", "クリップボードへのコピーに失敗しました。", parent=self
            )
        ...
        try:
            ...
            for full_date in self.selected_dates:
                if "%a" in format_str:
                    ...
                    formatted_text = full_date.strftime(temp_format).replace(
